...
public class ExtractMAQAlignments {
   private Hashtable idxToTRCID = null;
   private String idxFile = "/Users/kbrown/Documents/Work/SequenceOfLibrary/hairpin_genome.idx";
   ...
   public Hashtable writeResults(Hashtable results, String outFile) {
      ...
      try {
         ...
         while(it.hasNext()) {
            ...
            ExtractMAQAlignments.Read num = (ExtractMAQAlignments.Read)results.get(key);
            Integer keyInt = new Integer(key);
            ExtractMAQAlignments.Sequence s = (ExtractMAQAlignments.Sequence)this.idxToTRCID.get(keyInt);
      ...
      try {
         ...
         for(String line = null; (line = in.readLine()) != null; ++numReads) {
            ...
            if (!results.containsKey(new Integer(data[3]))) {
               results.put(new Integer(data[3]), new ExtractMAQAlignments.Read());
            } else {
               ((ExtractMAQAlignments.Read)results.get(new Integer(data[3]))).incrCount();
            }
         }
   ...
   private Hashtable readHairpinIndex(String inFile) {
      ...
      try {
         ...
         while((line = in.readLine()) != null) {
            ...
            idx = idx - 1;
            ExtractMAQAlignments.Sequence s = new ExtractMAQAlignments.Sequence(data[1], data[2]);
            h.put(idx, s);
   ...
   class Sequence {
      ...
      public boolean equals(Object s2) {
         return this.subpool == ((ExtractMAQAlignments.Sequence)s2).getSubpool();
      }
   }
}
